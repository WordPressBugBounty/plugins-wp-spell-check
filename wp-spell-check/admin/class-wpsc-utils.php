...
class Wpscx_Dashboard {
	...
	function create_dashboard_widget() {
		...
		if ( isset( $wpscx_check_opt ) && sizeof( $wpscx_check_opt ) >= 1 ) {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + hardcoded string. Query contains no user input.
			$word_count = $wpdb->get_var( 'SELECT COUNT(*) FROM ' . esc_sql( $table_name ) . ' WHERE ignore_word!=1' );
...
class Wpscx_OpenAI {
	...
	private function generateSEODescription( $text, $api_key ) {
		...
		// Execute the request using WordPress HTTP API
...
class Wpscx_Opendyslexic {
	...
	function profile_dyslexic( $user ) {
		...
		<table class="form-table" role="presentation">
			<tr>
				<td id="wpsc-opendyslexictable"><label><?php esc_html_e( 'Opendyslexic font', 'wp-spell-check' ); ?></label>
				</td>
				<td>
					<p><?php esc_html_e( 'You can use the OpenDyslexic font on the website or on both the website and the admin. The OpenDyslexic font is designed to help people with dyslexia with their reading. ', 'wp-spell-check' ); ?>
					</p>
				</td>
			</tr>
			<tr>
				...
				<td>
					<select name="wpsc_usedyslexic" id="wpsc_usedyslexic">
						<option value="no" <?php selected( 'no', get_user_meta( $user_id, 'wpsc_usedyslexic', true ) ); ?>>
							<?php esc_html_e( 'Do Not use the OpenDyslexic Font', 'wp-spell-check' ); ?>
						</option>
						<option value="yes_adminonly" <?php selected( 'yes_adminonly', get_user_meta( $user_id, 'wpsc_usedyslexic', true ) ); ?>>
							<?php esc_html_e( 'Use only on the admin area (back-end)', 'wp-spell-check' ); ?>
						</option>
						<option value="yes_websiteonly" <?php selected( 'yes_websiteonly', get_user_meta( $user_id, 'wpsc_usedyslexic', true ) ); ?>>
							<?php esc_html_e( 'Use only on the website (front-end)', 'wp-spell-check' ); ?>
						</option>
						<option value="yes_everywhere" <?php selected( 'yes_everywhere', get_user_meta( $user_id, 'wpsc_usedyslexic', true ) ); ?>>
							<?php esc_html_e( 'Use both on the website and Admin area', 'wp-spell-check' ); ?>
						</option>
					</select>
				</td>
			</tr>
	...
	function update_dyslexic( $user_id ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- WordPress core verifies nonce before hook fires
	...
	function dyslexic_css() {
		...
		// Enqueue CSS file early to prevent FOUC
		wp_enqueue_style(
			'wpsc-dyslexic',
			plugin_dir_url( __DIR__ ) . 'css/wpsc-dyslexic.css',
			array(),
			$wpsc_version
		);

		// Add dynamic @font-face with plugin URL
		$font_url      = plugin_dir_url( __FILE__ ) . 'OpenDyslexic-Regular.ttf';
		$font_face_css = "@font-face {
	font-family: open-dyslexic;
	src: url('" . esc_url( $font_url ) . "');
}";
		wp_add_inline_style( 'wpsc-dyslexic', $font_face_css );

		// Conditionally add font-family rule
		if ( 'yes_everywhere' === $use_opendyslexic || 'yes_websiteonly' === $use_opendyslexic ) {
			$font_family_css = '*:not(.ab-icon) {
	font-family: open-dyslexic, sans-serif !important;
}';
			wp_add_inline_style( 'wpsc-dyslexic', $font_family_css );
		}
	}
	...
	src: url('" . esc_url( $font_url ) . "');
		...
		wp_add_inline_style( 'wpsc-dyslexic-admin', $font_face_css );
	...
	font-family: open-dyslexic, sans-serif !important;
			...
			wp_add_inline_style( 'wpsc-dyslexic-admin', $font_family_css );
		}
	}
}
...
class Wpscx_Results_Utils {
	...
	function ignore_word( $ids ) {
		...
		foreach ( $ids as $id ) {
			...
			$check_word = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $table_name . ' WHERE word=%s AND ignore_word = true', $ignore_word ) );
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table names are safe
			$check_dict = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $dict_table . ' WHERE word=%s', $word ) );
			if ( sizeof( (array) $check_word ) <= 0 && sizeof( (array) $check_dict ) <= 0 ) {
				$wpdb->update( $table_name, array( 'ignore_word' => true ), array( 'id' => $id ) );
				$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . esc_sql( $table_name ) . ' WHERE id != %d AND word=%s', array( $id, addslashes( $word ) ) ) );
	...
	function ignore_word_empty( $ids ) {
		...
		$word_list[0]      = '';
		foreach ( $ids as $id ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_empty'
			$words       = $wpdb->get_results( $wpdb->prepare( 'SELECT word FROM ' . $table_name . ' WHERE id=%d;', sanitize_text_field( $id ) ) );
			$word        = $words[0]->word;
			$ignore_word = str_replace( "'", "\'", $word );
			$ignore_word = str_replace( "'", "\'", $ignore_word );
	...
	function add_to_dictionary( $ids ) {
		...
		foreach ( $ids as $id ) {
			...
			$ignore_word = str_replace( "'", "\'", $ignore_word );
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table names are safe
			$check = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $dictionary_table . ' WHERE word=%s', $word ) );
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table names are safe
			$ignore_check = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $table_name . ' WHERE word=%s AND ignore_word = true', $ignore_word ) );
		...
		for ( $x = 0; $x < sizeof( (array) $old_words ); $x++ ) {
			...
			if ( 'Post Content' === $page_types[ $x ] || 'Page Content' === $page_types[ $x ] || 'Media Description' === $page_types[ $x ] || 'WooCommerce Product' === $page_types[ $x ] || 'WooCommerce Variation' === $page_types[ $x ] || 'WooCommerce Coupon Content' === $page_types[ $x ] ) {
				...
				if ( sizeof( $meta_result ) >= 1 ) {
					...
					// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: {$wpdb->postmeta} uses WordPress core table reference
				...
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Group' === $page_types[ $x ] ) {
				...
				$wpdb->update( $table_name, array( 'post_title' => $updated_content ), array( 'ID' => $page_names[ $x ] ) );
				clean_post_cache( (int) $page_names[ $x ] );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Slide Title' === $page_types[ $x ] ) {
				...
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Caption' === $page_types[ $x ] ) {

				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'posts'
				$menu_result     = $wpdb->get_results( $wpdb->prepare( 'SELECT post_excerpt FROM ' . $table_name . ' WHERE ID=%s', $page_names[ $x ] ) );
				$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $menu_result[0]->post_excerpt ) );

				$wpdb->update( $table_name, array( 'post_excerpt' => $updated_content ), array( 'ID' => $page_names[ $x ] ) );
				clean_post_cache( (int) $page_names[ $x ] );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Content' === $page_types[ $x ] ) {

				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'posts'
				$menu_result     = $wpdb->get_results( $wpdb->prepare( 'SELECT post_content FROM ' . $table_name . ' WHERE ID=%s', $page_names[ $x ] ) );
				$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $menu_result[0]->post_content ) );

				$wpdb->update( $table_name, array( 'post_content' => $updated_content ), array( 'ID' => $page_names[ $x ] ) );
				clean_post_cache( (int) $page_names[ $x ] );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Image Title' === $page_types[ $x ] ) {

				if ( ! filter_var( get_post_meta( $page_names[ $x ], 'ml-slider_inherit_image_title', true ), FILTER_VALIDATE_BOOLEAN ) ) {
					$field_content   = get_post_meta( $page_names[ $x ], 'ml-slider_title', true );
					$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $field_content ) );
					$updated_content = sanitize_text_field( $updated_content );
					update_post_meta( $page_names[ $x ], 'ml-slider_title', $updated_content );
				}
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Image Alt' === $page_types[ $x ] ) {

				if ( ! filter_var( get_post_meta( $page_names[ $x ], 'ml-slider_inherit_image_alt', true ), FILTER_VALIDATE_BOOLEAN ) ) {
					$field_content   = get_post_meta( $page_names[ $x ], '_wp_attachment_image_alt', true );
					$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $field_content ) );
					$updated_content = sanitize_text_field( $updated_content );
					update_post_meta( $page_names[ $x ], '_wp_attachment_image_alt', $updated_content );
				}
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Meta Slider Link Alt' === $page_types[ $x ] ) {

				$field_content   = get_post_meta( $page_names[ $x ], 'ml-slider_link-alt', true );
				$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $field_content ) );
				$updated_content = sanitize_text_field( $updated_content );
				update_post_meta( $page_names[ $x ], 'ml-slider_link-alt', $updated_content );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'spellcheck_words'
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			...
			$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
				...
				$yoast_meta_key = wpscx_yoast_page_type_to_postmeta_key( $page_types[ $x ] );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'posts'
				$page_result = $wpdb->get_results( $wpdb->prepare( 'SELECT ID, post_title FROM ' . $table_name . ' WHERE ID=%s', $page_names[ $x ] ) );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: constructed from $wpdb->prefix + 'postmeta'
				$desc_result = $wpdb->get_results( $wpdb->prepare( 'SELECT meta_value FROM ' . $meta_table . ' WHERE post_id=%d AND meta_key=%s', $page_result[0]->ID, $yoast_meta_key ) );

				if ( false !== strpos( $yoast_meta_key, 'focuskeywords' ) || false !== strpos( $yoast_meta_key, 'keywordsynonyms' ) ) {
					$updated_content = wpscx_yoast_apply_fix_to_json( $yoast_meta_key, $desc_result[0]->meta_value, $old_words[ $x ], $new_words[ $x ] );
				} else {
					$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $desc_result[0]->meta_value ) );
				}
				...
				$field_key = $tax_info['field_key'];
				$taxonomy  = $tax_info['taxonomy'];
				$content   = isset( $tax_meta[ $taxonomy ][ $term_id ][ $field_key ] ) ? $tax_meta[ $taxonomy ][ $term_id ][ $field_key ] : '';
				if ( false !== strpos( $field_key, 'focuskeywords' ) || false !== strpos( $field_key, 'keywordsynonyms' ) ) {
					$updated_content = wpscx_yoast_apply_fix_to_json( $field_key, $content, $old_words[ $x ], $new_words[ $x ] );
				} else {
				...
				$post_type_slug = wpscx_yoast_archive_post_type_from_label( $page_names[ $x ] );
				if ( null === $post_type_slug && '' !== $page_names[ $x ] && '0' !== $page_names[ $x ] ) {
					$post_type_slug = $page_names[ $x ];
				}
				if ( null !== $post_type_slug ) {
					$titles     = get_option( 'wpseo_titles', array() );
					$option_key = $archive_prefix . $post_type_slug;
					if ( isset( $titles[ $option_key ] ) ) {
						$titles[ $option_key ] = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $titles[ $option_key ] ) );
						update_option( 'wpseo_titles', $titles );
					}
				}
				$wpdb->query( $wpdb->prepare( "DELETE FROM $words_table WHERE id=%d", $word_id ) );
			} elseif ( 'Yoast Author Pronouns' === $page_types[ $x ] ) {
				$author_result   = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $user_table WHERE user_id=%d AND meta_key='wpseo_pronouns'", $page_names[ $x ] ) );
				$updated_content = preg_replace( wpscx_regex_pattern( $old_words[ $x ] ), $new_words[ $x ], html_entity_decode( $author_result[0]->meta_value ) );
				$wpdb->update(
					$user_table,
					array( 'meta_value' => $updated_content ),
	...
	function update_empty_admin( $new_words, $page_names, $page_types, $old_word_ids ) {
		...
		for ( $x = 0; $x < sizeof( (array) $new_words ); $x++ ) {
			if ( empty( $page_names ) || empty( $new_words ) ) {
				continue;
			}
			...
			if ( 'Media Description' === $page_types[ $x ] ) {
			...
			} elseif ( 'WooCommerce Product Excerpt' === $page_types[ $x ] ) {
				...
				$wpdb->update( $table_name, array( 'post_excerpt' => $updated_content ), array( 'ID' => $page_names[ $x ] ) );
				$wpdb->delete( $words_table, array( 'id' => $old_word_ids[ $x ] ) );
			} elseif ( 'Menu Item' === $page_types[ $x ] || 'Post Title' === $page_types[ $x ] || 'Page Title' === $page_types[ $x ] || 'WooCommerce Product Name' === $page_types[ $x ] ) {
