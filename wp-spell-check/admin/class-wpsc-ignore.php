...
class Wpscx_Ignore_Table extends WP_List_Table {
	...
	function prepare_items() {
		...
		if ( isset( $_GET['s'] ) && '' !== $_GET['s'] ) {
			$search_term = str_replace( "'", "'", sanitize_text_field( wp_unslash( $_GET['s'] ) ) );

			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe
			$results = $wpdb->get_results( $wpdb->prepare( 'SELECT id, word FROM ' . $table_name . ' WHERE ignore_word=true AND word LIKE %s', '%' . $wpdb->esc_like( $search_term ) . '%' ), OBJECT );
		}
		$data = array();
	...
	function prepare_empty_items() {
		...
		if ( isset( $_GET['s'] ) ) {
			...
			// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: $wpdb->prefix . 'spellcheck_empty', query uses $wpdb->prepare()
...
function wpscx_ignore_render() {
	...
	if ( '' !== $delete && isset( $_GET['word'] ) && strpos( sanitize_text_field( wp_unslash( $_GET['word'] ) ), ' - ' ) !== false ) {
		if ( check_admin_referer( 'unignore-id-' . $delete ) ) {
	...
	if ( isset( $_POST['submit'] ) && 'Add to Ignore List' === $_POST['submit'] ) {
		...
		$words             = explode( PHP_EOL, sanitize_textarea_field( wp_unslash( $_POST['words-ignore'] ) ) );
		$message           = '';
		...
		foreach ( $words as $word ) {
			...
			if ( strlen( $word ) > 1 ) {
				...
				$check_word = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $table_name . ' WHERE word=%s AND ignore_word = true', $word ) );
				// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table names are safe
	...
	$list_table = new Wpscx_Ignore_Table();
	$list_table->prepare_items();

	$empty_table = new Wpscx_Ignore_Table();
	...
	<div class="wrap wpsc-table wpsc-ignore-page">
		...
		<div class="wpsc-scan-nav-bar">
			...
			?>
			name="wpsc-general-options">Spellcheck
				Words</a>
		...
		?>
		>
			<p style="font-size: 18px; font-weight: bold;">Here you can add words to the ignore list. Words here will not be
				flagged as incorrectly spelled words during a scan of your website.</p>
			<form action="admin.php?page=wp-spellcheck-ignore.php" name="add-to-ignore" id="add-to-ignore" method="POST">
				...
				<label>Words to ignore(Place one on each line)</label><br /><textarea name="words-ignore" rows="4"
			...
			<form method-"post"=""
				...
				<p class="search-box">
					...
					<input type="submit" id="search-submit" class="button" value="Search My Ignore List">
				</p>
			</form>
		</div>
