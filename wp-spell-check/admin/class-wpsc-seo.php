...
class Wpscx_Seo_Scanner extends wpscx_scanner {
	...
	function check_yoast_page_empty_free( $rng_seed = 0, $is_running = false, $log_debug = true ) {
		...
		$seo_check = $wpdb->get_results( 'SELECT post_id, meta_value, meta_key FROM ' . $table_name . ' WHERE (meta_key="_yoast_wpseo_metadesc" OR ' . wpscx_yoast_postmeta_sql_or_clause( array_keys( wpscx_aioseo_postmeta_desc_keys() ) ) . ' OR ' . wpscx_yoast_postmeta_sql_or_clause( array_keys( wpscx_rank_math_postmeta_desc_keys() ) ) . ') GROUP BY post_id' );
		++$sql_count;
	...
	function check_seo_titles_page_empty_free( $rng_seed = 0, $is_running = false, $log_debug = true ) {
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Table name is safe: $wpdb->prefix . 'postmeta', no user input
		$seo_check = $wpdb->get_results( 'SELECT post_id, meta_value, meta_key FROM ' . $table_name . ' WHERE (meta_key="_yoast_wpseo_title" OR ' . wpscx_yoast_postmeta_sql_or_clause( array_keys( wpscx_aioseo_postmeta_title_keys() ) ) . ' OR ' . wpscx_yoast_postmeta_sql_or_clause( array_keys( wpscx_rank_math_postmeta_title_keys() ) ) . ') GROUP BY post_id' );
		++$sql_count;
